  ...
  describe("rsa crypto") {
    ...
    it("should be strip leading zeros in exported jwk") {
      for {
        ...
        decodedKey = {
          println(s"exported jwk ${exportedJWK}")
          upickle.default.read[JwkKey](exportedJWK)
        }
        decodedBytes = {
          val deco = Base64.getUrlDecoder.decode(decodedKey.n)
          println(s" bytes  length ${bytes.length}")
          println(s"dec bytes  length ${deco.length}")
          deco
        }
